...
def load_data():
    try:
        ai_df = pd.read_csv('data/AIMtechStudentsData.csv',
                            skiprows=1, encoding='latin1')
        ds_df = pd.read_csv('data/DSMtechStudensData.csv',
                            encoding='latin1')
...
def create_positive_histogram(data, column, title, color=primary_color,
                              min_value=None):
    ...
    ax.text(mean_val * 1.01, ax.get_ylim()[1] * 0.9,
            f'Mean: {mean_val:.2f}',
...
def create_positive_scatter(data, x_col, y_col, title):
    ...
    p = np.poly1d(z)
    ax.plot(data[x_col], p(data[x_col]), color=primary_color, linewidth=1.5,
            linestyle='--')
...
def main():
    ...
    if combined_df.empty:
        st.error("Failed to load data. Please check the data files and "
                 "try again.")
        return

    st.sidebar.header("🔍 Quick Filters")
    program_filter = st.sidebar.multiselect("Program", ['AI', 'DS'],
                                            default=['AI', 'DS'])
    ...
    with col1:
        st.markdown(f'<div class="metric-card"><h3>👨‍🎓 Total Candidates'
                    f'</h3><p>{len(filtered_df)}</p></div>',
                    unsafe_allow_html=True)
    with col2:
        avg_cgpa = filtered_df['CGPA'].mean()
        st.markdown(f'<div class="metric-card"><h3>📈 Avg. CGPA</h3><p>'
                    f'{f"{avg_cgpa:.2f}" if not pd.isna(avg_cgpa) else "N/A"}'
    ...
    st.markdown("### <span style='color:black'>📚 Academic Profile</span>",
                unsafe_allow_html=True)

    col1, col2 = st.columns(2)
    with col1:
        with st.container():
            st.markdown('<div class="plot-container"><h3>CGPA Distribution '
                        '(≥6.0)</h3>', unsafe_allow_html=True)
            fig = create_positive_histogram(filtered_df, 'CGPA', "",
                                            min_value=6.0)
            st.pyplot(fig)
            st.markdown('</div>', unsafe_allow_html=True)

        with st.container():
            st.markdown('<div class="plot-container"><h3>Top 5 UG Streams'
                        '</h3>', unsafe_allow_html=True)
            if 'UG Stream' in filtered_df:
                fig = create_positive_barplot(filtered_df, None, 'UG Stream',
                                              "", top_n=5)
                st.pyplot(fig)
            else:
    ...
    with col2:
        # Removed the PG Semester Performance plot and kept only the
        # Performance by Program plot
        with st.container():
            st.markdown('<div class="plot-container"><h3>Performance by '
                        'Program</h3>', unsafe_allow_html=True)
            fig, ax = plt.subplots(figsize=(6, 4))
    ...
    st.markdown("### <span style='color:black'>💼 Professional Experience "
                "Insights</span>", unsafe_allow_html=True)
        ...
        with cols[0]:
            with st.container():
                st.markdown('<div class="plot-container"><h3>Internship '
                            'Participation</h3>', unsafe_allow_html=True)
                fig = create_donut_chart(filtered_df, internship_col, "",
                                         colors=[secondary_color, '#e5e7eb'])
                st.pyplot(fig)
                st.markdown('</div>', unsafe_allow_html=True)

            with st.container():
                st.markdown('<div class="plot-container"><h3>Top Internship '
                            'Roles</h3>', unsafe_allow_html=True)
                if 'Internship Role' in intern_df.columns:
                    fig = create_positive_barplot(intern_df, None,
                                                  'Internship Role', "",
                                                  palette="Greens_r")
                    st.pyplot(fig)
                else:
    ...
    if ft_col and ft_col in filtered_df:
        ...
        with cols[1]:
            with st.container():
                st.markdown('<div class="plot-container"><h3>Full-time '
                            'Experience</h3>', unsafe_allow_html=True)
                fig = create_donut_chart(filtered_df, ft_col, "",
                                         colors=[primary_color, '#e5e7eb'])
                st.pyplot(fig)
                st.markdown('</div>', unsafe_allow_html=True)

            with st.container():
                st.markdown('<div class="plot-container"><h3>Top Full-Time '
                            'Roles</h3>', unsafe_allow_html=True)
                if 'Full Time Role' in ft_df.columns:
                    fig = create_positive_barplot(ft_df, None,
                                                  'Full Time Role', "",
                                                  palette="Blues_r")
                    st.pyplot(fig)
                else:
    ...
    st.markdown("### <span style='color:black'>🔄 Experience vs "
                "Academics</span>", unsafe_allow_html=True)

    col3, col4 = st.columns(2)
    with col3:
        with st.container():
            st.markdown('<div class="plot-container"><h3>CGPA vs Internship'
                        'Status</h3>', unsafe_allow_html=True)
    ...
    with col4:
        with st.container():
            st.markdown('<div class="plot-container"><h3>Experience by '
                        'Program</h3>', unsafe_allow_html=True)
            if internship_col and 'Program' in filtered_df:
                fig, ax = plt.subplots(figsize=(6, 4))
                sns.countplot(x='Program', hue=internship_col,
                              data=filtered_df, ax=ax,
                              palette=[secondary_color, '#e5e7eb'])
                ax.set_xlabel("Program", color=text_color)
                ax.set_ylabel("Count", color=text_color)
                ax.tick_params(colors=text_color)
                ax.spines['bottom'].set_color(text_color)
                ax.spines['left'].set_color(text_color)
                ax.legend(title="Internship", bbox_to_anchor=(1.05, 1),
                          loc='upper left')
                plt.tight_layout()
                st.pyplot(fig)
            else:
